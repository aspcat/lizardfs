...
uint64_t IoLimitsDatabase::setAllocation(ClientId id, const GroupId& groupId, const uint64_t goal) {
...
uint64_t IoLimitsDatabase::getAllocation(ClientId id, const GroupId& groupId) {
	GroupState& group = getGroup(groupId);
	Allocation& allocation = getAllocation(group, id);
	return allocation;
}
